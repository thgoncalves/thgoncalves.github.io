...
Python is a very intuitive and flexible programming language. With it, you can tackle all types of problems that might come your way. In this article, I decided to post the most common snippets I use on my work as a Data Scientist.
<!--more-->

In this post, I'll show 10 snippets that can help you solve simple problems easier using Python and Jupyter Labs. There is no particular order here, just a collection of snippets.
...
Sometimes we just want unique values from a list. There is a simple way to do that and it is using SET. On Python Sets.
...
We can also use Sets to check if there are duplications on our list by checking the length of the original list and the length of that list. If they are the same, no duplications exist.
...
Slicing is an important function to apply on list in general. It is used to select parts of the list rather than the entire object.
...
Personally, I use a lot a #Print# statements while working on Python and sometimes I need a way to separate all the print that is being displayed.
...
Sometimes we want to add variables to a print statement along with some explanatory text. Before Python 3, we used to write the print like this:
...
There are 3 main variations of the Magic Function _time_. Bellow, I'll explain the difference between them:
1) %%time (two %)
...
2) %time (one %)
  - This also displays execution time but only for the code that precedes it and not for the entire cell
...
3) %timeit
  - This one is a little bit different. It evaluates the code over multiple runs and depends on the code itself.
...
Going back to the print function, there are a few handy options that may be of help.
